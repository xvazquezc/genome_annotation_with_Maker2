---
title: "Advanced repeat library for Maker2"
...
> __NOTE__: a usual installation of HMMER is required. ProteinExcluder depends on the `easl` miniapps from HMMER and neither Conda or Ubuntu installs them.
...
### Filter transposases from SwissProt {#filter_sprot}
...
> __IMPORTANT__: LTR harvest doesn't like certain special characters, including "." and "_", in the fasta headers. It also splits the headers at the spaces.

### Recent LTRs (99%) {#newltr}
...
RepeatModeler is able to identify some repeats but not other. Let's separate them and keep processing the _unknowns_:
```{bash, eval=FALSE}
...
The last step involves evaluating if some of this _unknown_ repeats are just fragments of genes mistakenly detected as repeats:
```{bash, eval=FALSE}
